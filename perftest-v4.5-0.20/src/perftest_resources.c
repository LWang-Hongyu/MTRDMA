...
int set_valid_dek(char *dst, struct perftest_parameters *user_param)
{
	...
	size = strlen(user_param->data_enc_key_app_path) + 1 + strlen(user_param->kek_path) + 1 + strlen(file_path) + 1;
								   ...
								   struct perftest_parameters *user_param)
{
	...
	// printf("\033[31mphx test post_send: wr_id=%lu, length=%d\033[0m\n", ctx->wr[index*user_param->post_list].wr_id, ctx->wr[index*user_param->post_list].sg_list[0].length);
										...
										int qp_index)
{
	...
	if ((!(user_param->duplex || user_param->tst == LAT) && (user_param->machine == SERVER)) || ((user_param->duplex || user_param->tst == LAT) && (qp_index >= num_of_qps)))
	{
...
int alloc_ctx(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	else if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) && (user_param->verb == SEND || user_param->verb == WRITE_IMM) && user_param->machine == SERVER)
	{
				...
				struct perftest_parameters *user_param)
{
	...
	for (i = 0; i < user_param->num_of_qps; i++)
	{
		...
		if ((((user_param->connection_type == DC && !((!(user_param->duplex || user_param->tst == LAT) && user_param->machine == SERVER) || ((user_param->duplex || user_param->tst == LAT) && i >= num_of_qps))) ||
...
cq_ex_not_supported:
#endif
	ctx->send_cq = ibv_create_cq(ctx->context, tx_buffer_depth * user_param->num_of_qps, NULL, ctx->send_channel, user_param->eq_num);
	if (!ctx->send_cq)
	{
		fprintf(stderr, "Couldn't create CQ\n");
		return FAILURE;
	...
	if (need_recv_cq)
	{
		ctx->recv_cq = ibv_create_cq(ctx->context, user_param->rx_depth * user_param->num_of_qps, NULL, ctx->recv_channel, user_param->eq_num);
		if (!ctx->recv_cq)
		{
			fprintf(stderr, "Couldn't create a receiver CQ\n");
			return FAILURE;
...
int ctx_init(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	if (user_param->use_srq && user_param->connection_type != DC &&
		!user_param->use_xrc && (user_param->tst == LAT || user_param->machine == SERVER || user_param->duplex == ON))
	{
								...
								int qp_index)
{
	...
	if (user_param->connection_type != RawEth)
	{
		...
		if (((attr->ah_attr.port_num == user_param->ib_port) && (user_param->gid_index == DEF_GID_INDEX)) || ((attr->ah_attr.port_num == user_param->ib_port2) && (user_param->gid_index2 == DEF_GID_INDEX) && user_param->dualport))
		{
				...
				struct pingpong_dest *my_dest)
{
	...
	for (i = 0; i < user_param->num_of_qps; i++)
	{
		...
		if (user_param->connection_type == DC)
		{
			if (((!(user_param->duplex || user_param->tst == LAT) && (user_param->machine == SERVER)) || ((user_param->duplex || user_param->tst == LAT) && (i >= user_param->num_of_qps / 2))))
			{
				continue;
			}
		}
		   ...
		   (user_param->test_type == DURATION && user_param->state != END_STATE))
	{
		...
		for (index = 0; index < num_of_qps; index++)
		{
				   ...
				   !((user_param->rate_limit_type == SW_RATE_LIMIT) && is_sending_burst == 0))
			{
				...
				if (user_param->post_list == 1 && (ctx->scnt[index] % user_param->cq_mod == 0 && user_param->cq_mod > 1) && !(ctx->scnt[index] == (user_param->iters - 1) && user_param->test_type == ITERATIONS))
				{
				...
				// printf("\033[31mrun_bw_iter phx test post_send: wr_id=%lu, length=%d\033[0m\n", ctx->wr[index*user_param->post_list].wr_id, ctx->wr[index*user_param->post_list].sg_list[0].length);
		...
		printf("\033[31mout of starting poll cq!!!\033[0m\n");
		...
		if (totccnt < tot_iters || (user_param->test_type == DURATION && totccnt < totscnt))
		{
			...
			printf("\033[31mstart poll cq!!!\033[0m\n");
...
int run_iter_lat_write(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	while (scnt < user_param->iters || ccnt < user_param->iters || rcnt < user_param->iters || ((user_param->test_type == DURATION && user_param->state != END_STATE)))
	{
...
int run_iter_lat_burst(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	while (totrcnt < (totscnt / user_param->reply_every) || totccnt < tot_iters)
	{
		...
		while ((totscnt < user_param->iters) && (totscnt - totccnt) < (user_param->tx_depth) && !(is_sending_burst == 0))
		{
...
int run_iter_fs(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	do
	{ /* This loop runs once in Iteration mode */
						   ...
						   struct perftest_parameters *user_param, struct rdma_addrinfo *hints)
{
	...
	if (user_param->connection_type == UD || user_param->connection_type == RawEth)
